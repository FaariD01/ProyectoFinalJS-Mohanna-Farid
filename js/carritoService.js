...
function agregarAlCarrito(producto){
  ...
  if(!memoria || memoria.length === 0) {
    ...
    localStorage.setItem("materialesConstruccion",JSON.stringify([nuevoProducto]));
    ...
    localStorage.setItem("materialesConstruccion",JSON.stringify(nuevaMemoria));
    actualizarNumeroCarrito();
...
function actualizarNumeroCarrito(){
  let cuenta = 0;
  const memoria = JSON.parse(localStorage.getItem("materialesConstruccion"));
  if(memoria && memoria.length > 0){
...
function reiniciarCarrito(){
  localStorage.removeItem("materialesConstruccion");
  actualizarNumeroCarrito();
}
